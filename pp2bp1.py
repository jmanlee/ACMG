...
def check_gene_pathogenic_mechanism(
    ...
    Note:
        ...
        - Missense: 아래 기준 모두 포함
            ...
            2. 전체 variant 중 50% 이상이 pathogenic.
            3. pathogenic variant 중, pathogenic missense가 50% 이상.

        - Null variant: 아래 기준 모두 포함.
        {nonsense, frameshift variant, splice donor variant, splice acceptor variant}
            1. pathogenic null variant 2개 이상
            2. 전체 variant 중 50% 이상이 pathogenic.
            3. pathogenic variant 중, pathogenic null variant가 50% 이상.
    ...
    total_variant_count = len(gene_variant_dic)
    ...
    if (
        pathogenic_missense_count >= 2
        and pathogenic_count / total_variant_count > 0.5
        and pathogenic_missense_count / pathogenic_count > 0.5
    ):
        return "missense variant"
    elif (
        pathogenic_null_count >= 2
        and pathogenic_count / total_variant_count > 0.5
        and pathogenic_null_count / pathogenic_count > 0.5
    ):
        return "null variant"
...
def execute(
    ...
    Note: ACMG rule 중에서, pp2/bp1 에 해당하는 룰을 구현한 모듈이다. 각각의 rule에 대한 설명은 다음과 같다.
        ...
        missense 변이
        bp1: Truncating variant (null variant)가 질병의 주된 원인이라고 보고된 유전자에서의
        missense 변이
        ...
        본 모듈의 작동방식은 다음과 같다.
        먼저, 각 variant에 대하여 missense variant인지 아닌지 조사한다(is_missense_var()). 만약 해당 변이의 전사체가 missense variant라면, 해당 변이의 gene_symbol과 일치하는 ClinVar 데이터들을 조사하여, 해당 유전자에서
        발생하는 질병의 원인이 missense variant인지, null variant인지 계산한다(check_gene_pathogenic_mechanism()). 만약, 해당 유전자에서 주로 pathogenic 한 변이가 보고되고, missense 변이가 일반적인 원인이면 pp2를 부여하며, null variant가 질병의 주된 원인이라면 bp1을 부여한다(assign_pp2bp1_rule()).
