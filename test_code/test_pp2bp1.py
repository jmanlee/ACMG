...
def clinvar_db_dic():
    ...
    clinvar_db_dic = {
        "CYP2C19": {"10-96521657-C-A": ["Benign", {"-": 1}, "-"]},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("OAT", ""),
        ("PLCE1", "null variant"),
        ("ECEL1", "missense variant"),
    ],
)
def test_check_gene_pathogenic_mechanism(
    gene_symbol, expected, clinvar_db_dic, clinvar_col2idx
):

    assert expected == check_gene_pathogenic_mechanism(
...
def test_assign_pp2bp1_rule(
    gene_symbol, expected, clinvar_db_dic, clinvar_col2idx
):

    assert expected == assign_pp2bp1_rule(
